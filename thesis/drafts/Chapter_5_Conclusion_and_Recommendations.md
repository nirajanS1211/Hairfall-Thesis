...
4. CatBoost's final fit used the test partition to monitor early stopping, so the test data influenced the number of trees selected (Section 3.8.3). A separate validation split would be cleaner.
5. All results come from one train/test split with one random seed, and confidence intervals for the metrics were not reported.
